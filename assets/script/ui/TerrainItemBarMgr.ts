import { Component, instantiate, Node, Prefab, ScrollView, size, Sprite, SpriteAtlas, Toggle, ToggleContainer, tween, UITransform, v3, Vec3, _decorator } from 'cc'
...
export default class TerrainItemBarMgr extends Component {
  ...
  onTerrainItemSelected(event: Toggle) {
    ...
    this.selectedPropSnap.spriteFrame = atlas.getSpriteFrame(event.target.name)
  ...
  private loadTrrainScrollBarItems() {
    ...
    for (let i = 0; i < configs.length; ++i) {
      ...
      sprite.spriteFrame = atlas.getSpriteFrame(configs[i].name)
